...
const RegistrationModal = ({ isOpen, onClose, onRegister, camp }) => {
    ...
    const onSubmit = async (data) => {
        ...
        const campRes =await axiosSecure.post('/carts',participantDetails)
        if(campRes.data.insertedId){
            reset()
            Swal.fire({
                position:'top-end',
                title: `${name} is added to the Camp Collection`,
               showConfirmButton: false,
               timer: 1500
              })
        }

        onRegister(participantDetails, camp);
        onClose();
    };
    return (
        <div>
            <Modal isOpen={isOpen} onRequestClose={onClose} contentLabel="Registration Modal">
                <div className="flex justify-end text-black mt-5 ">
                    <button onClick={onClose} className="text-2xl btn text-red-500 cursor-pointer">
                        <RxCross2 />

                    </button>
                </div>

                <h2 className='text-black text-3xl underline'>Participant Registration for {camp.name}</h2>
                <form onSubmit={handleSubmit(onSubmit)}>

                    <div className="relative mt-5 h-10 text-black w-full min-w-[200px]">
                        <input
                    ...
                    <button type='submit' className='btn mt-5 w-full' >Register</button>
                </form>
